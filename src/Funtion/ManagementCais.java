...
public class ManagementCais extends JFrame implements ActionListener{
	private JPanel panel;
	private Util.MyAbstractTableModel model;
	private JTable table;
	private Object data1[][] = new Object[15][3];
	private JButton add=new JButton("增加菜式");
	private JButton delete=new JButton("删除菜式");
	private JButton change=new JButton("改变菜式");
	private JButton query=new JButton("搜索菜式");
	private JButton queryAll=new JButton("查看全部菜式");
	...
	public void actionPerformed(ActionEvent e) {
		...
		public void deleteData(){
			...
			try{
				...
				if(!rs.next()){
					JOptionPane.showMessageDialog(ManagementCais.this, "该菜式不存在","查询记录",1);
				}else{
					...
					JOptionPane.showMessageDialog(ManagementCais.this, "删除菜式成功","删除记录",1);
					...
					JOptionPane.showMessageDialog(ManagementCais.this, "删除失败，请输入菜名","删除记录",1);
				}
				}
		...
		public void queryByNameData(){
			...
			try{
				...
				if(caiName==null||caiName.equals("")){
					JOptionPane.showMessageDialog(ManagementCais.this, "查询失败，请输入菜名","查询记录",1);
					return;
				}
		...
		public void addData(){
			...
			try {
				...
				String sql="select * from t_chengpincais where caiName=?";
				 ...
				 rs=JdbcUtils.executeQuery(conn, sql, caiName);
				if(rs.next()){
					JOptionPane.showMessageDialog(ManagementCais.this, "该菜式已经存在，请详细查看后才操作","添加记录",1);
				}else{
					...
					JOptionPane.showMessageDialog(ManagementCais.this, "添加成功","添加记录",1);
					...
					JOptionPane.showMessageDialog(ManagementCais.this,"添加失败，请输入菜名或者价格","添加记录",1);
				}
				}
			} catch (SQLException e1) {
		...
		try{
			...
			rs=JdbcUtils.executeQuery(conn, sql, caiName);
			if(!rs.next()){
				JOptionPane.showMessageDialog(ManagementCais.this, "用修改的菜式不存在，请详细查看后才操作","添加记录",1);
			}else{
				...
				JOptionPane.showMessageDialog(ManagementCais.this, "修改成功","修改记录",1);
				...
				JOptionPane.showMessageDialog(ManagementCais.this, "修改失败，请输入菜名","修改记录",1);
			}
			}
		} catch (SQLException e1) {
		...
		public void queryAllData(){
			...
			try {
				...
				while(rs.next()){
					...
					//加入数据
					newVector.addElement(rs.getString("caiName")+"");
					newVector.addElement(rs.getInt("price"));
